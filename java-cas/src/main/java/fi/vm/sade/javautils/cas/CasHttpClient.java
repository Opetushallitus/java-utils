...
public class CasHttpClient {
    ...
    private boolean currentTokenCouldBeValid() {
        if (TOKEN_STORE.get() == null) {
            logger.info("Token is null - not valid.");
            return false;
        ...
        return callToFuture(this.client.newCall(requestTicketGrantingTicket))
                .thenCompose((tgtResponse) -> {
                    if (tgtResponse.isSuccessful()) {
                        HttpUrl ticketGrantingTicketUrl = HttpUrl.get(tgtResponse.headers("Location").stream().findFirst().get());
                                ...
                                .thenCompose((stResponse) -> {
                                    if (stResponse.isSuccessful()) {
                                        try {
                                            ...
                                            if (this.securityUriSuffix.equals("j_spring_cas_security_check")) {
                                                sessionRequestUrl = CasUtils.getHost(this.casUrl) + this.service + "/?ticket=" + stResponse.body().string();
                                            } else {
                                                sessionRequestUrl = CasUtils.getHost(this.casUrl) + this.service + "/" + this.securityUriSuffix + "?ticket=" + stResponse.body().string();
                                            }
                                            ...
                                            logger.info("GETTING SESSION FOR URL:" + sessionRequestUrl);

                                            return callToFuture(this.client.newCall(sessionRequest))
                                                    .thenCompose((sessionResponse) -> {
                                                        logger.info("got session response for service:" + this.service);
                                                        logger.info("got response code:" + sessionResponse.code());
    ...
    private CompletableFuture<Response> call(final Request request, final int tryNumber) {
        ...
        if (currentTokenCouldBeValid()) {
            logger.info("current token valid for request: " + request);
            Request requestWithSessionCookie = new Request.Builder(request)
                    .addHeader("Caller-Id", this.callerId)
                    .addHeader("Cookie", String.format("CSRF=%s;", this.callerId))
                    .addHeader("Cookie", this.cookieName + "=" + TOKEN_STORE.get().value)
                    .build();
            logger.info("request: " + requestWithSessionCookie.url());
            logger.info("request: " + requestWithSessionCookie.headers());
            return callToFuture(this.client.newCall(requestWithSessionCookie))
                    .thenCompose((response) -> {
                        if (response.code() == 200) {
                            logger.info("RESPONSE CODE 200 for request: " + requestWithSessionCookie.url());
                            unauthorized.set(false);
                        } else if (response.code() == 302) {
                            logger.info("RESPONSE CODE 302, for request: " + requestWithSessionCookie.url());
                            return fetchCasSession().thenCompose((casResponse) -> {
                                logger.info("SESSION FETCHED FOR REQUEST: " + request.url());
                                if (currentTokenCouldBeValid()) {
                                    unauthorized.set(false);
                            ...
                            logger.info("response code 401, fetching cas session for request: " + requestWithSessionCookie.url());
                            return fetchCasSession().thenCompose((casResponse) -> {
                                try {
                                    if (currentTokenCouldBeValid()) {
                                        unauthorized.set(false);
    ...
    public String validateServiceTicketWithVirkailijaUsername(String service, String ticket) {

        logger.info("VALIDATING TICKET: " + ticket + " , service: " + service);
        Request request = new Request.Builder()
                .url(this.casUrl + "/serviceValidate?ticket=" + ticket + "&service=" + service)
                .header("Caller-Id", this.callerId)
                .header("CSRF", this.callerId)
                .header("Cookie", String.format("CSRF=%s;", this.callerId))
                .build();
        try {
            ...
            throw new RuntimeException("CAS serviceValidate call failed.", e);
        }
    }
}